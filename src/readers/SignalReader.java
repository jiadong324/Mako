...
public class SignalReader {
    ...
    private BufferedWriter writer = null; // Used to write superitems.
...
//    private int superitemMaxSpanRange;
    ...
    private ChannelParser breakendChannel;
    ...
    public SignalReader(int fragMean, int fragStd, int cutStd, int readLen, int maxDist, int minMapQ, String fastaIndexFile) throws IOException{
...
//        superitemMaxSpanRange = 2 * fragMean;
     ...
     * @throws IOException 
     */
    public void doWork(String bamFile, String chrom, int chromStart, int chromEnd,
        ...
        breakendChannel = new ChannelParser(clusteringDist, "bnd", abnormalSigOut);
        
        if (!superitemOutPath.isEmpty()){
            createSuperItemWriter(superitemOutPath);            
        }

        if (!bndOutPath.isEmpty()) {
            bndWriter = new BufferedWriter(new FileWriter(bndOutPath));
        }

        extractSignalsFromBAM(bamFile, chrom, chromStart, chromEnd);
    ...
    private void extractSignalsFromBAM(String bamFile, String chrom, int regionStart, int regionEnd) throws IOException{
        ...
        if (chrom != null){
            ...
            for (int i = 0; i < nWindows; i++){  
                ...
                superitemCount += curBinSuperitemCount;
